...
class WebSocketServer {
    ...
    public $array_user=[];
    public $array_buttle=[];
    public function __construct($ip = '10.11.9.7', $port = 7777) {
        $this->ip = $ip;
        $this->port = $port;
    ...
    public function check_array(){
            $a=count($this->array_user);
            print($a);
            if($a>=2){
                foreach ($this->array_user as $key => $element) {
                    unset($this->array_user[$key]);
                }
            }
        }
    /**
        ...
        while (true) {
            ...
            foreach ($read as $connect) { // обрабатываем все соединения, в которых есть данные для чтения
                ...
                if (is_callable($this->handler)) {
                    ...
                    // $arr=explode(' ',$decoded['payload']);
                    // if($arr[0]=='login'){
                    //     array_push($this->array_user,$arr[1]);
                    // }
                    // if($arr[0]=='battle'){  
                       
                    //     if($this->array_buttle!=null){
                    //         array_push($this->array_buttle,$connect);
                    //         $this->response($this->array_buttle[0],$this->array_user[0]);
                    //         $this->response($arr[1],$this->array_user[1]);
                    //         foreach ($this->array_buttle as $key => $element) {
                    //             unset($this->array_buttle[$key]);
                    //         }
                    //         $this->check_array();
                    //     }
                    //     else {
                    //         array_push($this->array_battle,$connect);
                    //     }
                    // }
